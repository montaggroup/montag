...
# different structure header (PK 0x05 0x06) which python zipfile does not support natively
...
# noinspection PyAugmentAssignment,PyUnresolvedReferences,PyUnusedLocal
...
# noinspection PyMissingConstructor,PyArgumentList,PyTypeChecker,PyPep8
...
# noinspection PyAttributeOutsideInit,PyArgumentList,PyProtectedMember,PyAugmentAssignment,PyShadowingBuiltins,PyStringFormat,PyPep8
class ZipFile(object):
    ...
    def _RealGetContents(self):
        ...
        while total < size_cd:
            ...
            if self.debug > 2:
                print(centdir)
            filename = fp.read(centdir[_CD_FILENAME_LENGTH]).decode("utf-8")
    ...
    def open(self, name, mode="r", pwd=None):
        ...
        try:
            ...
            if fheader[_FH_SIGNATURE] != stringFileHeader and fheader[_FH_SIGNATURE] != stringEndArchive:
                raise BadZipfile("Bad magic number for file header %s %s", fheader[_FH_SIGNATURE], stringFileHeader)

            fname = zef_file.read(fheader[_FH_FILENAME_LENGTH]).decode("utf-8")
